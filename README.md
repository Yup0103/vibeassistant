...
`render.yaml` at the repo root defines a single web service that builds the frontend, builds the backend, and serves both from one origin (the backend serves `web/dist` as static files, with an SPA fallback to `index.html`) — this keeps every `/auth`/`/chat` request same-origin, so the session cookie works with no CORS complications.

1. Push this repo to GitHub (already done if you're reading this from the hosted repo).
2. In the Render dashboard: **New +** → **Blueprint**, point it at this repo. Render reads `render.yaml` automatically.
3. Render will ask you to fill in the two `sync: false` env vars it can't generate on its own:
   - `APP_USER_EMAIL` — your login email
   - `APP_USER_PASSWORD_HASH` — generate with `node -e "console.log(require('bcryptjs').hashSync('your-password', 10))"`

   `SESSION_SECRET` is auto-generated by Render; `MOCK_MODE`, `COOKIE_SECURE`, `DB_PATH`, and `KITE_MCP_URL` are already set in `render.yaml`.
4. Deploy. Once it's live, open the Render URL, log in, and it behaves exactly like the local mock-mode app.

**Note on persistence:** the free Render web service plan has an ephemeral filesystem — `data.sqlite` (and with it, chat history) resets on every redeploy or restart. The single-user login always still works because it's reseeded from `APP_USER_EMAIL`/`APP_USER_PASSWORD_HASH` on every boot; only conversation history is affected. Add a Render Disk mounted at the server's working directory if you want history to survive restarts.
